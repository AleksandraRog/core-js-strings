...
function removeLastOccurrences(str, value) {
  ...
  return ind < 0 ? str : str.slice(0, ind) + str.slice(ind + value.length);
}
...
function sumOfCodes(str) {
  return isString(str)
    ? str.split('').reduce((sum, val) => sum + val.charCodeAt(), 0)
    : 0;
}
...
function countVowels(str) {
  return `${str}`.match(/[aeiouyAEIOUY]/g)
    ? str.match(/[aeiouyAEIOUY]/g).length
    : 0;
}
...
function isPalindrome(str) {
  return str
    ...
    .reduce(
      (result, val, i, arr) =>
        val.toLowerCase() !== arr[arr.length - i - 1].toLowerCase()
          ? false
          : result,
      true
    );
...
function invertCase(str) {
  return str
    .split('')
    .map((val) => (val.match(/[A-Z]/g) ? val.toLowerCase() : val.toUpperCase()))
    .join('');
}
...
function encodeToRot13(str) {
  return str.replace(/[a-zA-Z]/g, function charCode(char) {
